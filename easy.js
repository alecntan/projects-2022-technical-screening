...
const altNumbers = (numArray) => {
		...
		let firstArr = posNums;
		let secArr = negNums;

		if(negNums.length > posNums.length) { firstArr = negNums; secArr = posNums; }
		
		let res = [];
		while( firstArr.length > 0 || secArr.length > 0 ){
			if(firstArr.length > 0) {res.push(firstArr.shift());}
			if(secArr.length > 0) {res.push(secArr.shift());}
		}
    return res;
}
